...
class TextFiLM(nn.Module):
    ...
    Inputs
    ------
    point_feat : [B, C, N]   backbone point features
    ...
    def __init__(self, point_dim: int = 512, text_dim: int = 512):
        super().__init__()
        hidden = point_dim
        self.mlp = nn.Sequential(
            nn.Linear(text_dim, hidden),
            nn.ReLU(inplace=True),
            nn.Linear(hidden, point_dim * 2),   # -> gamma [C] + beta [C]
        )
        # init: identity transform at start of training
        nn.init.zeros_(self.mlp[-1].weight)
        nn.init.zeros_(self.mlp[-1].bias)

    def forward(self, point_feat: torch.Tensor, scene_feat: torch.Tensor) -> torch.Tensor:
        # [B, C*2]  ->  gamma [B, C, 1], beta [B, C, 1]
        params = self.mlp(scene_feat)
        gamma, beta = params.chunk(2, dim=-1)
        gamma = gamma.unsqueeze(-1) + 1.0   # residual: start as identity
